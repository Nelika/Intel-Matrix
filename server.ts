...
function sanitizeCisaLink(rawLink?: string, advisoryId?: string): string {
  const id = (advisoryId || "").toLowerCase();
  let link = (rawLink || "").trim();

  // Fix medical advisory URL path if it used /ics-advisories/icsma-...
    ...
    try {
      ...
      const parsedItems: CisaAdvisoryItem[] = rawItems.slice(0, 35).map((raw, idx) => {
        ...
        return {
          ...
          advisoryId,
          title,
          link: sanitizeCisaLink(link, advisoryId),
          pubDate: pubDateIso,
          vendor,
